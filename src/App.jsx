    ...
    <main>
      ...
      <section id="projects" className="content-section software-section">
        ...
        <div className={`spotlight spotlight-${projects[activeProject].visual}`}>
          ...
          <div className="spotlight-caption"><h3>{projects[activeProject].title.toLowerCase()}</h3><p>{projects[activeProject].description}</p><span>{projects[activeProject].stack}</span><div className="spotlight-links"><a href={projects[activeProject].github} target="_blank" rel="noreferrer" aria-label={`${projects[activeProject].title} source code`} title="GitHub"><Icon name="github" /></a>{projects[activeProject].devpost && <a href={projects[activeProject].devpost} target="_blank" rel="noreferrer" aria-label={`${projects[activeProject].title} on Devpost`} title="Devpost"><Icon name="devpost" /></a>}{projects[activeProject].demo && <a href={projects[activeProject].demo} target="_blank" rel="noreferrer">Live app ↗</a>}</div></div>
        ...
        <div className="project-grid">{projects.map(item => <article className="project-card" key={item.title}><div className="project-card-top"><span className="folder-symbol">▱</span><div className="project-card-links"><a href={item.github} target="_blank" rel="noreferrer" aria-label={`${item.title} source code`} title="GitHub"><Icon name="github" /></a>{item.devpost && <a href={item.devpost} target="_blank" rel="noreferrer" aria-label={`${item.title} on Devpost`} title="Devpost"><Icon name="devpost" /></a>}</div></div><h3>{item.title}</h3><p>{item.description}</p><span className="project-stack">{item.stack}</span>{item.demo && <a className="project-demo" href={item.demo} target="_blank" rel="noreferrer">Live app ↗</a>}</article>)}</div>
      </section>
